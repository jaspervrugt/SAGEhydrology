function varargout = user_model(par,mdl,data,ode,check)
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%USER_MODEL Run a user-defined conceptual watershed model through the
% compiled modular SAGE user-model interface.
%
% SYNOPSIS:
%   q_n = user_model(par,mdl,data,ode)
%   [q_n,J] = user_model(par,mdl,data,ode)
%   [q_n,J,Jth] = user_model(par,mdl,data,ode)
%   [q_n,J,Jth,Z] = user_model(par,mdl,data,ode)
%   [...] = user_model(par,mdl,data,ode,check)
%
% INPUTS
%   par         d-by-1 parameter vector in the active parameter space
%
%   mdl         model information structure
%     .mcode      numerical implementation selector
%                 4 = compiled modular C++ user model
%     .y0         initial model state value(s)
%     .pspace     parameter space:
%                 0 = hydrologic
%                 1 = normalized unit cube
%                 2 = unconstrained
%     .th_min     d-by-1 lower parameter bounds
%     .th_max     d-by-1 upper parameter bounds
%     .par_names  parameter names
%     .tout       final model print time
%     .idx        start/end indices of the requested output period
%
%   data        meteorological forcing and auxiliary data structure
%     .P          precipitation
...
%     .Order      error-controller order
%     .maxiter    maximum consecutive rejected steps
%     .mem        state storage: 0 = streaming, 1 = full trajectory
%
%   check       retained for compatibility with the built-in model
%               interface. The deployed modular C++ implementation does
%               not currently use this argument.
%
% OUTPUTS
%   q_n         simulated discharge for the requested period
%   J           discharge Jacobian in the active parameter space
%   Jth         derivative of hydrologic parameters with respect to the
%               active parameter-space coordinates
%   Z           complete augmented state trajectory when ode.mem = 1;
%               final augmented state when ode.mem = 0
%
% NOTES
%   The numerical solver, parameter transformation, state preparation,
%   sensitivity propagation, memory-aware output handling, and failure
%   handling are implemented by:
%
%     crr_user_model_mex.cpp
%     user_model_prepare.cpp
%     user_model.cpp
%
%   The gateway requests only the outputs needed by the caller. Thus, a
%   discharge-only call does not allocate the complete Jacobian when
%   ode.mem = 0. For ode.mem = 1, discharge and sensitivities are derived
%   from the stored cumulative-discharge state.
%
% Written by J. A. Vrugt, May 2026
% Revised July 2026
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ~isstruct(mdl) ...
            || ~isfield(mdl,'mcode')
        error('user_model:InvalidModelStructure', ...
            ['mdl must be a structure ' ...
            'containing field mcode.']);
    end

    if mdl.mcode ~= 4
        error('user_model:OnlyMEX', ...
            ['The deployed modular user_model ' ...
            'supports only mcode = 4 ' ...
             '(compiled C++ implementation).']);
    end

    if ~isstruct(data)
        error('user_model:InvalidDataStructure', ...
            'data must be a structure.');
    end

    if ~isstruct(ode)
        error('user_model:InvalidODEStructure', ...
            'ode must be a structure.');
    end
    ...
    if ~isnumeric(par) ...
        ...
        || isempty(par) ...
        || ~isvector(par)
        error('user_model:InvalidParameters', ...
            ['par must be a nonempty ' ...
            'real numeric vector.']);
    end
    ...
    if ~isscalar(check) ...
            || ~(islogical(check) ...
            || isnumeric(check))
        error('user_model:InvalidCheck', ...
            ['check must be a scalar ' ...
             'logical or numeric value.']);
    end

    if exist('crr_user_model','file') ~= 3
        if isdeployed
            error('user_model:MissingDeployedMEX', ...
                ['The deployed SAGE application cannot compile a user ' ...
                 'model. Install a precompiled crr_user_model binary for ' ...
                 'this operating system and MATLAB Runtime release in ' ...
                 'the user_model folder.']);
        end
        error('user_model:MissingMEX', ...
            ['crr_user_model is not compiled. Run user_model_compile ' ...
             'from the user_model folder with a configured C++ compiler.']);
    end
...
%% function below was before change to GUI executable supported user model

% function varargout = user_model(par,mdl,data,ode,check)
% %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% %USER_MODEL: Runge Kutta implementation of Gr4j conceptual watershed model with
% % analytic routing 
% % SYNOPSIS: varargout = user_model(par,mdl,data,ode,check)
...
% %                 4 Runge Kutta implementation ode_user_model C++
...
%                 h = max(hmin_,min(h,hmax_)); h = min(h,t2-t);   % Cannot exceed t2-t
...
%         [~,Z] = ode45(@(t,z) user_model_aug_ode(t,z,th,data, ...
...
%                 dzdt = user_model_aug_ode(s-2,... % compute dzdt based on 
...
% 
%         [Z,q_n,J] = crr_user_model(mdl.tout,Z(1,1:nvar)',data,ode);
% 
% end
% 
% if fail == true && (mem == 1)
...
%     T_sm,eps_m,eps_s,rho,m,d)
% 
% dzdtE = user_model_aug_ode(t,z,th,data,U,dUdx4,L,eta,kappa,bg,bR,mts, ...
%     T_sm,eps_m,eps_s,rho,m,d)';                 % Euler
% zE = z + h*dzdtE;
% dzdtH = user_model_aug_ode(t,zE,th,data,U,dUdx4,L,eta,kappa,bg,bR,mts, ...
...
% function dzdt = user_model_aug_ode(t,z,th,data,U,dUdx4,L,eta,kappa,bg,bR, ...
%     mts,T_sm,eps_m,eps_s,rho,m,d)
% 
% x = z(1:m);                                     % x = [Swe S R U1 U2 Q]
% S = reshape(z(m+1:m*(d+1)),m,d);                % mxd sensitivity matrix
% [dxdt,dSdt] = user_model_odefcn(t,x,th,S,data, ...   % compute dxdt & dSdt
%     U,dUdx4,L,eta,kappa,bg,bR,mts,T_sm,eps_m, ...
%     eps_s,rho,m,d);
...
% end
% 
% function [dudt,dSdt,Jth_f,Jx_f] = user_model_odefcn(t,u,th,S,data, ...
%     U,dUdx4,L,~,kappa,bg,bR,mts,T_sm,eps_m,eps_s,~,m,d)
% %user_model_ODEFCN  ODE stores + analytic routing via UH convolution
% % (Mathias-style CIF)
% %
% % Notes
% % -----
...
% 
% % ---------------------------------------------
% % 6) snow module (from user_model, plus derivatives)
% % ---------------------------------------------
...
% Jth_f = zeros(m,d);
% 
% % --- physical-store Jacobian pieces (same logic as user_model, mapped) ---
