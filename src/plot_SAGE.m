...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    paperResultsOnly = isstruct(gaugescen) ...
        ...
        && logical(gaugescen.paper_results_only(1));
    ...
    try 
        ...
        if hasJKGE
            ...
            % JKGE is optional, but S_FDC is always reported. When JKGE is
        ...
        nZonePages = max(1,ceil( ...
            numel(zoneIDsForLayout)/zonesPerPage));
        plot_metric_ecdf_by_zone_figure(firstZoneFigure, ...
            part,mdl, ...
            dt_str,samp_word, ...
            scenariosNSE,n_m,id_model, ...
        ...
        % =======================================
        plot_metric_ecdf_by_zone_figure(firstZoneFigure+nZonePages, ...
            part,mdl, ...
            dt_str,samp_word, ...
            scenariosKGE,n_m,id_model, ...
            model_names,colors,figW,figH, ...
        ...
        % =========================================
        if hasJKGE
            plot_metric_ecdf_by_zone_figure(firstZoneFigure+2*nZonePages, ...
                part,mdl, ...
                dt_str,samp_word, ...
                scenariosJKGE,n_m,id_model, ...
                model_names,colors,figW,figH, ...
    ...
    if strcmpi(part,'sage') ...
        ...
        if paperResultsOnly
            fprintf('  plot_SAGE: parameter maps complete.\n');
        end
    end
...
%PLOT_METRIC_ECDF_FIGURE Plots scenario-specific ECDFs for one metric.
    ...
    % Use the same stable manuscript canvas as Paper-4 Figure 1. Deriving
    % width from screen pixels makes this figure extremely wide on a 4K
    % display while its height remains capped, producing huge panel gaps.
    figW = min(16.0,0.98 * figW);
    figH = min(figH,5.6);
    ...
    % Keep the descriptive plot_SAGE figure title while using the Paper-4
    % styling inside the four ECDF panels.
...
%PLOT_METRIC_ECDF_BY_ZONE_FIGURE Plots metric ECDFs by hydroclimatic zone.
    ...
    for page = 1:nPage
        ...
        for izPage = 1:nCol
            ...
            % Compact scenario row labels avoid overly tall rotated text and
            % remain valid during PPTX export.
            if izPage == 1
...
%   - darker color = larger normalized value
    ...
    for j = 1:d
        ...
        if isstruct(gaugescen) ...
                ...
                && ~isempty(gaugescen.paper_results_only) ...
                && logical(gaugescen.paper_results_only(1))
            % The manuscript parameter maps use country boundaries only.
            % Starting with the empty basemap also avoids an unnecessary
            % online tile request before the country outline is added.
            mapBase = 'none';
        end
        geobasemap(gax(j),mapBase);
...
%   - force point-style for pure random sampling
    ...
    for is = 1:numel(groups)
        ...
        for jf = 1:nFig
            ...
            % Use the same axes-relative label location for every panel.
            % Text objects do not inherit MATLAB's variable ylabel offset.
            for iRow = 1:numel(axh)
                local_position_timeseries_ylabels( ...
                    axForcing(iRow),axh(iRow),60);
            end
...
%LOCAL_PLOT_BASIN_POSTPROCESSOR Plots discharge for one basin and scenario.
    ...
    local_fixed_left_axis_label(axh,sprintf('$Q$ (%s)',q_unit), ...
        [0 0 0],fontsize_axis,60);
...
function hText = local_fixed_left_axis_label( ...
        ax,labelText,colorValue,fontSize,labelOffsetPixels)
%LOCAL_FIXED_LEFT_AXIS_LABEL Draw a stable axes-relative P or Q label.
% MATLAB's ylabel offset depends on tick-label extents and changes with the
% renderer, figure size, and panel count. A fixed pixel offset gives narrow
% multi-panel and wide single-panel layouts the same physical spacing.

    if nargin < 5 || isempty(labelOffsetPixels)
        labelOffsetPixels = 60;
    end
    old = findobj(ax,'Type','text','Tag','SAGEFixedLeftYLabel');
    delete(old(isgraphics(old)));
    try
        % Do not call YYAXIS on a discharge axes: doing so creates an
        % unintended orange right-hand ruler. Select the left ruler only
        % when this axes already is a genuine two-ruler forcing axes.
        if numel(ax.YAxis) > 1
            yyaxis(ax,'left');
        end
    catch
    ...
    hText = text(ax,-abs(labelOffsetPixels),0.5*p(4)-3,labelText, ...
        'Units','pixels', ...
        'Interpreter','latex', ...
        'FontSize',fontSize, ...
        'Color',colorValue, ...
...
function local_position_timeseries_ylabels( ...
    axForcing,axDischarge,labelOffsetPixels)
%LOCAL_POSITION_TIMESERIES_YLABELS Set final label spacing and font sizes.

    if nargin < 3 || isempty(labelOffsetPixels)
        labelOffsetPixels = 60;
    end
    ...
    if isgraphics(axDischarge)
        label = findobj(axDischarge,'Type','text', ...
            'Tag','SAGEFixedLeftYLabel');
        if ~isempty(label)
...
function symbol = local_metric_median_symbol(metricTag,scenarioTag)
...
%LOCAL_LATEX_ESCAPE Escapes special characters for LaTeX-rendered text.
    ...
    if isfield(Qfdc,'req') ...
        ...
        ipT = ipT(ipT >= 1 ...
            & ipT <= numel(Qfdc.gauge));
        if ~isempty(ipT)
            [tf,loc] = ismember(ipT,iqRet);
            pTrain = kRet(loc(tf));
        end
        ...
        ipE = ipE(ipE >= 1 ...
            & ipE <= numel(Qfdc.gauge));
        if ~isempty(ipE)
            [tf,loc] = ismember(ipE,iqRet);
            pEval = kRet(loc(tf));
        end
    end
...
%LOCAL_QFDC_HAS_REQUESTED_SCENARIO Tests whether Qfdc contains a requested scenario.
    ...
    if isempty(iq) || ~isscalar(iq) || ~isfinite(iq)
        tf = false;
        return
    end
